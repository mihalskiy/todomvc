...
const Menu: React.FC<Props> = ({ path }) => {
  ...
  const doneCount: number = appState.todoList.filter((t) => t.isComplete).length
  const yetCount: number = appState.todoList.filter((t) => !t.isComplete).length

  function clearCompleted(): void {
    setAppState({
      todoList: appState.todoList.filter((t: Todo) => !t.isComplete),
    })
  }
